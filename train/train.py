...
from transformers import (  # noqa
    ...
    AutoTokenizer,
    PreTrainedModel,
    PreTrainedTokenizerFast as PreTrainedTokenizer,
...
class Args:
    ...
    This is all the default inputs for main_train.
    """
...
class ConversationDataset(Dataset):
    """Holds the conversation data."""

    def __init__(
        self, tokenizer: PreTrainedTokenizer, args, df, block_size=512
    ):
        """Init the data set."""
...
def load_and_cache_examples(args, tokenizer, df_trn, df_val, evaluate=False):
    """Use to load the cached data."""
    return ConversationDataset(tokenizer, args, df_val if evaluate else df_trn)


def set_seed(args):
    """Set a nice random seed."""
    random.seed(args.seed)
    np.random.seed(args.seed)
    torch.manual_seed(args.seed)
    if args.n_device > 0 and not args.no_cuda:
...
def _rotate_checkpoints(
    ...
    for checkpoint in checkpoints_to_be_deleted:
        logger.info(
            ...
            "[{}] due to args.save_total_limit".format(checkpoint)
        )
...
def main_train(df_trn, df_val, **kwargs):
    """Do the training."""
    args = Args()
    for kwarg in kwargs:
    ...
    if args.no_cuda:
        ...
        args.n_device = torch.cuda.device_count()
    args.device = device
    ...
    logger.warning(
        "Process rank: %s, device: %s, n_device: %s, distributed training: %s,"
        " 16-bits training: %s",
        args.local_rank,
        device,
        args.n_device,
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(
                args, model, tokenizer, df_trn, df_val, prefix=prefix
            )
            result = dict(
...
if __name__ == "__main__":
    ...
    if "<output>" in arguments:
        args["output_dir"] = arguments["<output>"]

    if in_ipython_kernel:  # Google colabs
        args["no_cuda"] = False

    n = 7

    lines = list(filter(None, input_path.read_text().split("\n")))

    contexted = []

    for i in range(n, len(lines)):
        row = []
        # we additionally substract 1, so row will contain current
        # and 7 previous responces
        prev = i - 1 - n
